...
struct ScansListView : View {
    ...
    @ObservedObject var vm : ScanListViewModel
    ...
    var body : some View {
        ...
        VStack(spacing:20){
            ...
            List {
                ForEach(vm.products, id: \.id) { photo in
                    if let image = FileStorageManager.shared.loadImage(named: photo.fileName ?? "") {
                                                
                        Button {
                            coordinator.goToDetail(product:photo)
                        } label: {
...
struct AddExpense_Preview : PreviewProvider {
    static var previews : some View {
        ...
        ScansListView(coordinator:AppCoordinator(), vm:ScanListViewModel(productsUseCase:useCase))
    }
}
